...
class Command(CSVBaseCommand):
    ...
    def _process_row(self, row, simulate=False, overwrite=False, **options):
        ...
        if any([pipeline_item.sector.pk != old_sector_id, pipeline_item.sector.pk == new_sector_id]):
            logger.warning(
                f'Not updating pipeline item {pipeline_item} as its sector has not changed',
            )
            return
