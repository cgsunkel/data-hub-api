...
def test_delete_documents(es_bulk, mock_es_client):
    ...
    index = 'test-index'
    es_docs = [
        {'_type': DEFAULT_DOC_TYPE, '_id': 1},
        {'_type': DEFAULT_DOC_TYPE, '_id': 2},
        {'_type': DEFAULT_DOC_TYPE, '_id': 3},
    ]
...
def test_delete_documents_with_errors(es_bulk, mock_es_client):
    ...
    index = 'test-index'
    es_docs = [{'_type': DEFAULT_DOC_TYPE, '_id': 1}]
...
def test_collector(monkeypatch, es_with_signals):
    ...
    assert es_with_signals.count(read_alias, doc_type=DEFAULT_DOC_TYPE)['count'] == 1

    collector.delete_from_es()

    es_with_signals.indices.refresh()
    assert es_with_signals.count(read_alias, doc_type=DEFAULT_DOC_TYPE)['count'] == 0
